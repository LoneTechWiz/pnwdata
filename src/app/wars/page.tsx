          ...
          <ExportButton
            ...
            getData={() => wars.map(w => {
              ...
              return {
                ...
                "Att Soldiers": w.attacker?.soldiers ?? 0,
                "Att Tanks": w.attacker?.tanks ?? 0,
                "Att Aircraft": w.attacker?.aircraft ?? 0,
                "Att Ships": w.attacker?.ships ?? 0,
                "Def Soldiers": w.defender?.soldiers ?? 0,
                "Def Tanks": w.defender?.tanks ?? 0,
                "Def Aircraft": w.defender?.aircraft ?? 0,
                "Def Ships": w.defender?.ships ?? 0,
                "Att Ground Control": Number(w.ground_control) === w.att_id ? "Yes" : "No",
                "Att Air Superiority": Number(w.air_superiority) === w.att_id ? "Yes" : "No",
                "Att Blockade": Number(w.naval_blockade) === w.att_id ? "Yes" : "No",
                "Def Ground Control": Number(w.ground_control) === w.def_id ? "Yes" : "No",
                "Def Air Superiority": Number(w.air_superiority) === w.def_id ? "Yes" : "No",
                "Def Blockade": Number(w.naval_blockade) === w.def_id ? "Yes" : "No",
                Reason: w.reason,
              };
            })}
          />
